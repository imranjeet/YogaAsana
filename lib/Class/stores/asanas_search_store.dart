...
abstract class AsanasSearchStoreBase with Store {
  ...
  AsanasSearchStoreBase({@required this.initialAsanasList})
      : assert(initialAsanasList != null) {
    _searchTextReaction =
        reaction((_) => _searchText, (text) => _updateAsanasBySearchText(text));

    _searchProgressReaction =
        reaction((_) => _isSearchInProgress, (isInProgress) {
